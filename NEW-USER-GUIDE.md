...
Every task gets a place in space and a reading in time, and PANDEMONIUM uses both to put it exactly where it should run, exactly when it should run. That's the entire thesis. The rest of this document is the six ideas that make it work, and the people and decades they came from.
...
**How PANDEMONIUM uses it.** A workload is a stream of numbers too — how busy the machine is, moment to moment. PANDEMONIUM's adaptive layer runs those three measures over a short window every second to recognize what *kind* of moment the system is in: idle, a mix, or flat-out saturated. That reading lets it shift strategy to match reality instead of reacting blindly to a single noisy number — and lets it tell a genuinely steady workload apart from one that just looks calm for a tick.

### 6. BPF/eBPF and sched_ext → where all of this runs
...
- **The Adaptive layer** lives in an ordinary program and wakes once a second. It reads what the BPF layer measured, figures out the workload regime (idle/mixed/saturated, idea #5), and re-tunes the kernel layer's knobs. It does the *learning*, using a technique called **multiplicative weight updates** — a panel of competing "expert" strategies, each betting on a set of settings, with the panel's trust shifted toward whichever expert is winning right now. Fast decisions below, slow learning above.

There's one more piece that ties it together over time: a **persistent process database**. Once a program has proven what it is — a snappy interactive app, a background batch job, a latency-critical thread — PANDEMONIUM remembers, and writes it to disk. The next time that program starts, it's placed correctly *immediately*, instead of being figured out cold all over again.

## Putting it back together

So, the whole machine in one picture. A task wakes up. PANDEMONIUM knows two things about it: a place in **space** (which cache neighborhood it belongs to — resistance affinity, idea #1) and a reading in **time** (how long it's been waiting — sojourn, idea #2). The kernel layer (idea #6) places it on the right core. A self-tuning controller (idea #4) keeps the timing honest and rests when the system is idle. When work piles up unevenly, a priced flow (idea #3) routes the overflow along the cheapest paths. A learning layer reading the workload's texture (idea #5) re-tunes the whole thing once a second, and a database remembers what it learned.

Six fields of mathematics and decades of other people's work — graph theory, queue control, maximum flow, feedback control, nonlinear dynamics, and in-kernel programming — condensed into one decision, made thousands of times a second: *where does this task go, right now?*
