...
def cpu_release_deprecated() -> bool:
    """True on kernels where sched_ext deprecated ops.cpu_release() and reworked
    scx_bpf_reenqueue_local() into a deferred async re-enqueue (the for-6.19 series,
    Linux 7.1+). Flooding cpu_release on these kernels hard-locks the box."""
    return kernel_at_least(7, 1)
