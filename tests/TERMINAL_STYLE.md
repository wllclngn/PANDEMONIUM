...
A full report (a montauk --analyze `--digest`, a prism-scale table block) is the
...
- trace analysis, offenders, cross-CCX, waits/spins → `montauk --analyze`.
- the report is a `montauk --analyze --digest`, consolidated by the suite and
...
number that a table helper, sublimation, or montauk --analyze should produce.
