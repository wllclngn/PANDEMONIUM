...
impl CpuTopology {
    pub fn detect(nr_cpus: usize) -> Result<Self> {
        ...
        for cpu in 0..nr_cpus {
            ...
            if let Some(id) = seen_groups.iter()
                .position(|g| g.contains(&(cpu as u32)))
            {
    ...
    pub fn detect_and_populate(sched: &mut Scheduler, nr_cpus: usize,
                               phi_scale: Option<u64>) -> Result<TopologySpectrum> {
        ...
        if let Err(e) = topo.populate_affinity_rank_map(
            ...
            spectrum.codel_eq_ns, &domain_phi) {
        ...
        log_info!("OVERFLOW DOMAINS: {} (emergent, cpu_domain populated)",
                         ov_domains);
        if let Err(e) = sched.write_topology_fields(
            spectrum.tau_ns, spectrum.codel_eq_ns) {
    ...
    pub fn poll_hotplug(sched: &mut Scheduler, nr_cpus: usize,
                        phi_scale: Option<u64>, last_online: &mut usize) -> bool {
        let now = Self::online_cpu_count();
        if now == 0 || now == *last_online {
            return false;
        }
        log_info!("HOTPLUG: online CPUs {} -> {} -- re-deriving topology",
                         *last_online, now);
        *last_online = now;
    ...
    fn fiedler_sweep_cut(
        ...
        ordered.sort_by(|&a, &b| {
            fvec[a].partial_cmp(&fvec[b]).unwrap_or(std::cmp::Ordering::Equal)
        });
    ...
    fn domain_cut(&self, members: &[usize]) -> Option<(Vec<usize>, Vec<usize>, f64)> {
        ...
        order.sort_by(|&a, &b| {
            fsub[a].partial_cmp(&fsub[b]).unwrap_or(std::cmp::Ordering::Equal)
        });
    ...
    pub fn compute_resistance_affinity(&self) -> (Vec<f64>, Vec<u32>, TopologySpectrum) {
        ...
        (reff, rank, TopologySpectrum { fiedler, tau_ns, codel_eq_ns, phi_dist_scale_q16 })
    }
        ...
        for cpu in 0..self.nr_cpus {
            for slot in 0..valid {
                ...
                let spill_d = if phi_dist_scale_q16 == 0 {
                    ...
                    let frac = (reff[cpu * self.nr_cpus + val as usize] / max_reff)
                        .clamp(0.0, 1.0);
    ...
    pub fn log_resistance_affinity(
        &self,
        reff: &[f64],
        rank: &[u32],
        ...
        if n >= 2 {
            ...
            log_info!(
                ...
                reff[l2_sib],
                reff[non_l2],
                if reff[l2_sib] > 0.0 { reff[non_l2] / reff[l2_sib] } else { 0.0 }
            );
        }
    }
...
mod t2_cut_tests {
    ...
    fn overflow_partition_matches_l3_groups() {
        ...
        assert!(mono.iter().all(|&d| d == 0), "target 1 = one domain: {:?}", mono);
    }
}
