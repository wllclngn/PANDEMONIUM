...
pub fn monitor_loop(
    ...
    while !shutdown.load(Ordering::Relaxed) && !sched.exited() {
        ...
        if delta_d > 0 {
            ...
            let target = (interval_ns * SOJOURN_MULTIPLIER).clamp(SOJOURN_FLOOR_NS, SOJOURN_CEIL_NS);
        ...
        {
            let current = sched.read_tuning_knobs();
            if current.batch_slice_ns != final_batch || current.sojourn_thresh_ns != sojourn_thresh_ns {
        ...
        let burst_label = if stats.burst_mode_active > 0 { " BURST" } else { "" };
