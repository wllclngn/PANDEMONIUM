...
use crate::tuning::{
    self, detect_regime, scaled_regime_knobs, Regime, HIST_BUCKETS,
};
...
impl LoadGraph {
    pub fn build<const N: usize>(depth_win: &[RawWindow<N>]) -> LoadGraph {
        ...
        let node_depth = depth_win
            .iter()
            .map(|w| if w.is_empty() { f64::NAN } else { chaos::mean(w) })
            .collect();
        ...
        let node_burst = depth_win.iter().map(chaos::kim_jo_burstiness_priced).collect();
        let node_slowing = depth_win.iter().map(chaos::lag1_autocorr_priced).collect();
        let node_persist = depth_win.iter().map(chaos::veitch_abry_hurst_priced).collect();

        let mut edge = vec![None; n * (n.saturating_sub(1)) / 2];
        for i in 0..n {
            for j in (i + 1)..n {
                edge[pair_index(n, i, j)] =
                    chaos::pecora_carroll(&depth_win[i], &depth_win[j]);
            }
        }
        LoadGraph { node_depth, node_burst, node_slowing, node_persist, edge, n }
    }
    ...
    pub fn derive_percpu_knobs(
        ...
        let present: Vec<f64> = self.node_depth.iter().copied().filter(|d| d.is_finite()).collect();
        ...
        self.node_depth
            ...
            .map(|(i, d)| {
                ...
                if let Some(b) = self.node_burst.get(i).and_then(|v| *v) {
                    ...
                    let f = Priced { value: 1.0 + 0.5 * b.value.clamp(-1.0, 1.0),
                                     confidence: b.confidence }.weighted(1.0);
                ...
                if let Some(r1) = self.node_slowing.get(i).and_then(|v| *v) {
                    if r1.value > 0.0 {
                        let tighten = Priced { value: 1.0 - 0.4 * r1.value.clamp(0.0, 1.0),
                                               confidence: r1.confidence }.weighted(1.0);
                        k.preempt_thresh_ns =
                            ((k.preempt_thresh_ns as f64) * tighten) as u64;
                    }
                }
                ...
                if let Some(h) = self.node_persist.get(i).and_then(|v| *v) {
                    let f = Priced { value: (1.0f64 - (h.value.clamp(0.0, 1.0) - 0.5)).clamp(0.5, 1.5),
                                     confidence: h.confidence }.weighted(1.0);
...
pub fn monitor_loop(
    ...
    while !shutdown.load(Ordering::Relaxed) && !sched.exited() {
        ...
        if CpuTopology::poll_hotplug(sched, nr_cpus as usize, phi_scale,
                                     &mut last_online) {
        ...
        let _scatter_pct = if delta_d > 0 { delta_scatter * 100 / delta_d } else { 0 };
        ...
        if prev_percpu.len() == percpu.len() {
            for (i, w) in depth_win.iter_mut().enumerate() {
                ...
                let ds = percpu[i].rq_depth_samples
                    .wrapping_sub(prev_percpu[i].rq_depth_samples);
                if ds > 0 {
                    let dd = percpu[i].rq_depth_sum
...
mod load_graph_tests {
    ...
    fn edge_summary_spread_is_the_falsification() {
        ...
        assert_eq!(n, 3);
        assert!(hi - lo > 0.3, "mixed machine should show spread, got {}", hi - lo);
    }
    ...
    fn nodes_absent_where_a_cpu_has_no_series() {
        ...
        assert!(g.node_depth[1].is_nan(), "an unsampled CPU must not read as 0");
    }
}
...
mod derive_tests {
    ...
    fn a_uniform_machine_is_a_no_op() {
        ...
        for k in g.derive_percpu_knobs(&b) {
            assert_eq!(k.slice_ns, b.slice_ns, "uniform depth must not move the slice");
    ...
    fn a_deep_queue_gets_a_shorter_slice_than_a_shallow_one() {
        ...
        assert!(k[0].slice_ns < k[1].slice_ns,
                "deep queue {} must slice shorter than mid {}", k[0].slice_ns, k[1].slice_ns);
        assert!(k[1].slice_ns < k[2].slice_ns,
                "mid {} must slice shorter than shallow {}", k[1].slice_ns, k[2].slice_ns);
    }
    ...
    fn fields_the_graph_does_not_own_are_carried_through() {
        ...
        let b = base();
        for k in g.derive_percpu_knobs(&b) {
            assert_eq!(k.codel_thresh_ns, b.codel_thresh_ns,
...
mod derivation_tests {
    ...
    fn bursty_traffic_lengthens_the_batch_ceiling_paced_shortens_it() {
        ...
        let b = base();
        let k = g.derive_percpu_knobs(&b);
        assert!(k[0].batch_slice_ns > b.batch_slice_ns,
                "bursty CPU should widen the batch ceiling: {} vs {}",
                k[0].batch_slice_ns, b.batch_slice_ns);
        assert!(k[1].batch_slice_ns < b.batch_slice_ns,
    ...
    fn critical_slowing_tightens_the_preempt_window() {
        ...
        let k = g.derive_percpu_knobs(&base());
        assert!(k[0].preempt_thresh_ns < k[1].preempt_thresh_ns,
    ...
    fn persistence_rescues_sooner_than_mean_reversion() {
        ...
        let mut s = 7u64;
        let mut acc = 5.0;
        let walk: Vec<f64> = (0..CHAOS_WIN).map(|_| { acc += lcg(&mut s); acc.abs() + 1.0 }).collect();
        let mut s2 = 11u64;
        let noise: Vec<f64> = (0..CHAOS_WIN + 1).map(|_| lcg(&mut s2)).collect();
        let diff: Vec<f64> = (1..=CHAOS_WIN).map(|i| 5.0 + (noise[i] - noise[i - 1])).collect();
        let g = LoadGraph::build(&[win(&walk), win(&diff)]);
        let k = g.derive_percpu_knobs(&base());
        assert!(k[0].codel_thresh_ns < k[1].codel_thresh_ns,
    ...
    fn the_sensors_move_different_knobs() {
        ...
        let mut s = 5u64;
        let a: Vec<f64> = { let mut v = vec![0.2f64; CHAOS_WIN]; v[4] = 20.0; v };
        let mean_a: f64 = a.iter().sum::<f64>() / a.len() as f64;
        let b: Vec<f64> = (0..CHAOS_WIN).map(|_| mean_a + 0.001 * lcg(&mut s)).collect();
        ...
        assert!(d * 10_000 < k[0].slice_ns,
                "equal mean depth must derive the same slice, got {} vs {}",
                k[0].slice_ns, k[1].slice_ns);
        assert_ne!(k[0].batch_slice_ns, k[1].batch_slice_ns,
    ...
    fn data_flows_below_the_old_gate() {
        ...
        let b = base();
        assert_ne!(g.derive_percpu_knobs(&b)[0].batch_slice_ns, b.batch_slice_ns,
    ...
    fn hurst_stays_gated_because_its_floor_is_arithmetic() {
        ...
        let b = base();
        assert_eq!(g.derive_percpu_knobs(&b)[0].codel_thresh_ns, b.codel_thresh_ns);
    }
    ...
    fn a_uniform_machine_derives_uniformly_but_not_necessarily_the_base() {
        ...
        for w in k.windows(2) {
            assert_eq!(w[0].slice_ns, w[1].slice_ns, "identical CPUs diverged on slice");
            assert_eq!(w[0].preempt_thresh_ns, w[1].preempt_thresh_ns,
                       "identical CPUs diverged on preempt");
            assert_eq!(w[0].batch_slice_ns, w[1].batch_slice_ns,
                       "identical CPUs diverged on batch ceiling");
            assert_eq!(w[0].codel_thresh_ns, w[1].codel_thresh_ns,
...
mod affinity_tests {
    ...
    fn coupling_no_longer_moves_affinity() {
        ...
        let g = LoadGraph::build(&[win(&a), win(&b)]);
        assert!(g.mean_coupling().unwrap() > 0.9, "the pair must still READ as coupled");
