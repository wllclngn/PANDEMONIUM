...
pub fn detect_regime(mean_idle_pct: f64, hvg_lambda: f64, bp_h: f64) -> Regime {
    let chaos_low = hvg_lambda < crate::chaos::HVG_LAMBDA_CHAOTIC_MIN
        || bp_h < crate::chaos::BP_H_HIGH;
...
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum IoBucket { Low, Mid, High }

pub fn io_bucket(io_pct: u64) -> IoBucket {
    if io_pct > 60 { IoBucket::High }
    else if io_pct < 15 { IoBucket::Low }
    else { IoBucket::Mid }
}
...
impl MwuController {
    ...
    pub fn update(
            ...
            match cur_io {
                IoBucket::High => {
                    ...
                    for i in 0..N_EXPERTS {
                        if i != EX_IO_HEAVY { losses[i] += v * 0.8; }
                    }
                    has_loss = true;
                }
                IoBucket::Low => {
        ...
        let fork_thresh = scale_tau_u64(tau_ns, K_FORK_STORM_RATE_Q16)
            .max(FORK_STORM_RATE_FLOOR);
        ...
        let lambda_cross = sig.hvg_lambda >= crate::chaos::HVG_LAMBDA_CHAOTIC_MIN
            && !self.prev_lambda_chaotic;
