						        ...
						        bool feed_oscillator)
{
	...
	if (serve_interactive) {
		...
		if (b_aged && scx_bpf_dsq_move_to_local(batch_dsq)) {
		...
		if (i_aged && scx_bpf_dsq_move_to_local(node_dsq)) {
			if (scx_bpf_dsq_nr_queued(node_dsq) == 0) {
				u64 old = interactive_enqueue_ns;
				if (old > 0)
					__sync_val_compare_and_swap(&interactive_enqueue_ns, old, 0);
...
// THE LOCAL DSQ VIA scx_bpf_dsq_move_to_local() GET STUCK, TRIGGERING
// THE WATCHDOG.
