...
impl<'a> Scheduler<'a> {
    ...
    pub fn read_reff_value(&self, cpu: u32, slot: u32) -> u32 {
        let stride = crate::bpf_intf::MAX_AFFINITY_CANDIDATES;
        let key = (cpu * stride + slot).to_ne_bytes();
        match self.skel.maps.reff_value.lookup(&key, libbpf_rs::MapFlags::ANY) {
...
mod fold_tests {
    ...
    fn counters_sum_across_cpus() {
        let cpus = [cpu(10, 0, 0, [0; 8]), cpu(7, 0, 0, [0; 8]), cpu(3, 0, 0, [0; 8])];
    ...
    fn sojourn_takes_the_max_not_the_sum() {
        ...
        let cpus = [cpu(0, 100, 0, [0; 8]), cpu(0, 500, 0, [0; 8]), cpu(0, 300, 0, [0; 8])];
    ...
    fn longrun_flag_takes_the_max_not_the_sum() {
        ...
        let cpus = [cpu(0, 0, 1, [0; 8]), cpu(0, 0, 1, [0; 8]), cpu(0, 0, 0, [0; 8])];
...
mod knob_broadcast_tests {
    ...
    fn global_fields_are_broadcast_from_slot_zero() {
        let mut v = vec![knobs(100, 7_000, 32), knobs(200, 9_999, 64), knobs(300, 1, 8)];
