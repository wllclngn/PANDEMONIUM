...
pub fn pecora_carroll<const N: usize>(
    x: &RawWindow<N>,
    y: &RawWindow<N>,
) -> Option<f64> {
...
// Unfloored body -- the priced form supplies its own minimum.
fn pecora_carroll_raw<const N: usize>(
    x: &RawWindow<N>,
    y: &RawWindow<N>,
) -> Option<f64> {
...
pub fn lag1_autocorr_priced<const N: usize>(w: &RawWindow<N>) -> Option<Priced> {
    ...
    Some(Priced { value: v, confidence: confidence(n, ACF1_MATH_MIN, ACF1_MIN_SAMPLES) })
}
...
pub fn kim_jo_burstiness_priced<const N: usize>(w: &RawWindow<N>) -> Option<Priced> {
    ...
    Some(Priced { value: v, confidence: confidence(n, BURSTINESS_MATH_MIN, BURSTINESS_MIN_SAMPLES) })
}

// Coupling needs a nearest neighbour that is not the point itself, so n >= 3.
const PC_MATH_MIN: usize = 3;
pub fn pecora_carroll_priced<const N: usize>(
    x: &RawWindow<N>,
    y: &RawWindow<N>,
) -> Option<Priced> {
    let n = x.filled.min(y.filled);
    ...
    Some(Priced { value: v, confidence: confidence(n, PC_MATH_MIN, PC_MIN_SAMPLES) })
}

// Hurst: gated, not priced. See above.
pub fn veitch_abry_hurst_priced<const N: usize>(w: &RawWindow<N>) -> Option<Priced> {
    veitch_abry_hurst(w).map(|v| Priced { value: v, confidence: 1.0 })
}
...
mod tests {
    ...
    fn acf1_white_noise_is_memoryless() {
        ...
        assert!(r.abs() < 0.3, "uncorrelated samples should not persist, got r1={r}");
    }
    ...
    fn acf1_alternating_is_negative() {
        let vals: Vec<f64> = (0..32).map(|i| if i % 2 == 0 { 1.0 } else { -1.0 }).collect();
        let r = lag1_autocorr(&fill::<32>(&vals)).expect("32 samples is enough");
    ...
    fn burstiness_regular_is_negative() {
        ...
        assert!(b < -0.5, "perfectly regular traffic should read regular, got B={b}");
    }
    ...
    fn burstiness_ordering_holds() {
        ...
        let (r, l, b) = (kim_jo_burstiness(&regular).unwrap(),
                         kim_jo_burstiness(&longrun).unwrap(),
                         kim_jo_burstiness(&bursty).unwrap());
        assert!(r < l && l < b, "expected regular {r} < longrun {l} < bursty {b}");
    }
    ...
    fn burstiness_rejects_non_positive_series() {
        ...
        let centered: Vec<f64> = (0..16).map(|i| if i % 2 == 0 { 1.0 } else { -1.0 }).collect();
    ...
    fn hurst_needs_a_full_window() {
        let w = fill::<64>(&[1.0; 8]);
        assert!(veitch_abry_hurst(&w).is_none(),
    ...
    fn hurst_white_noise_near_half() {
        ...
        assert!((0.25..=0.75).contains(&h), "uncorrelated sequence read H={h}");
    }
    ...
    fn hurst_ramp_is_persistent() {
        ...
        let w = fill::<64>(&vals);
        let h = veitch_abry_hurst(&w).expect("64 samples is enough");
        assert!(h > 0.75, "a monotonic ramp should read persistent, got H={h}");
    }
    ...
    fn hurst_pure_alternation_cannot_be_fit() {
        ...
        let pure: Vec<f64> = (0..64).map(|i| if i % 2 == 0 { 1.0 } else { -1.0 }).collect();
        ...
        assert!(h < 0.35, "a noisy alternation should mean-revert, got H={h}");
    }
    ...
    fn hurst_ordering_holds() {
        ...
        let (a, r, p) = (veitch_abry_hurst(&fill::<64>(&diff)).unwrap(),
                         veitch_abry_hurst(&rnd).unwrap(),
                         veitch_abry_hurst(&ramp).unwrap());
        assert!(a < r && r < p, "expected anti-persistent {a} < random {r} < ramp {p}");
    }
    ...
    fn hurst_random_walk_is_persistent() {
        ...
        let mut seed = 7u64;
        let mut acc = 0.0;
        let walk: Vec<f64> = (0..64).map(|_| { acc += lcg(&mut seed); acc }).collect();
    ...
    fn pecora_carroll_independent_series_do_not() {
        ...
        assert!(c < 0.5, "independent streams should not read coupled, got {c}");
    }
...
mod saturation_probe {
    ...
    fn a_pegged_box_satisfies_both_chaos_terms_of_the_freeze_gate() {
        ...
        assert!(lambda <= HVG_LAMBDA_PERIODIC_MAX,
                "flat idle_pct gives lambda={lambda}, inside the periodic band");
        assert!(det >= RQA_DET_STEADY_MIN,
...
mod pricing_tests {
    ...
    fn confidence_ramps_from_the_arithmetic_minimum_to_the_old_gate() {
        assert_eq!(confidence(2, 2, 8), 0.0, "at the minimum, nothing is trusted");
        ...
        assert!((mid - 0.5).abs() < 1e-9, "halfway is half weight, got {mid}");
    }
    ...
    fn weighted_scales_the_same_value_by_trust() {
        ...
        let full = Priced { value: 1.5, confidence: 1.0 };
        let half = Priced { value: 1.5, confidence: 0.5 };
        let none = Priced { value: 1.5, confidence: 0.0 };
        assert!((full.weighted(1.0) - 1.5).abs() < 1e-9);
        assert!((half.weighted(1.0) - 1.25).abs() < 1e-9);
        assert!((none.weighted(1.0) - 1.0).abs() < 1e-9,
    ...
    fn priced_estimators_answer_below_their_old_floors() {
        ...
        let mut w = RawWindow::<16>::new();
        for v in [0.1, 9.0, 0.1] { w.push(v); }
        assert!(kim_jo_burstiness(&w).is_none(), "the gated form still refuses");
        let p = kim_jo_burstiness_priced(&w).expect("the priced form answers");
        assert!(p.confidence > 0.0 && p.confidence < 1.0,
                "and prices it below full, got {}", p.confidence);
